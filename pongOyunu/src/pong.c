...
static void move_paddle_ai(SDL_SCANCODE_F1) {
...
static void move_paddle_ai1(SDL_SCANCODE_F2) {

	int center = paddle[0].y + 25;
	int screen_center = screen->h / 2 - 25;
	int ball_speed = ball.dy + 5;
...
static void move_paddle_ai2(SDL_SCANCODE_F3) {

	int center = paddle[0].y + 25;
	int screen_center = screen->h / 2 - 25;
	int ball_speed = ball.dy + 10 ;
	if (ball_speed < 0) {
...
int main(int argc, char* args[]) {
	...
	while (quit == 0) {
		...
		SDL_FillRect(screen, NULL, 0xFF120a8f);
...
int init(int width, int height, int argc, char* args[]) {
	...
	SDL_SetColorKey(logo, SDL_TRUE, colorkey);
